            ...
            for name in z:
                if str(name).endswith(".xml"):
                    ...
                    for oobj in root.findall('object'):
                        ...
                        point0_list = list(map(lambda x:float(x),point0.strip(',').split(',')))
                        point1_list = list(map(lambda x:float(x),point1.strip(',').split(',')))
                        point2_list = list(map(lambda x:float(x),point2.strip(',').split(',')))
                        point3_list = list(map(lambda x:float(x),point3.strip(',').split(',')))
                        ...
                        delta = (xmax-xmin)/dis*1.0
                        if abs(delta) < 0.00001:
                            logging.warning("invalid delta ")
                            continue
                        for h in range(dis):
                            xx = str(round(xmin + h * delta, 2))
                            yy = str(round(ymin, 2))
                            ww = str(round(delta, 2))
                            hh = str(round(ymax-ymin, 2))
